...
internal class EnemySpawnParamsDrawer : PropertyDrawer
{
    ...
    private static bool ShowPickButton(SerializedProperty property)
    {
        ...
        return posType == SpawnPositionType.Specific || posType == SpawnPositionType.RandomFromSpecific;
    }
    ...
    private static void OnSceneGUI(SceneView sceneView)
    {
        ...
        var spawnPoints = Object.FindObjectsByType<EnemySpawnPoint>();
        var e = Event.current;
    ...
    private static void ApplyAssignment(SerializedProperty property, EnemySpawnPoint spawnPoint)
    {
        ...
        if (posType == SpawnPositionType.Specific)
        {
            BindExposedReference(property.FindPropertyRelative("spawnPoint"), spawnPoint, director);
        }
        else if (posType == SpawnPositionType.RandomFromSpecific)
        {
            ...
            var newItem = internalItems.GetArrayElementAtIndex(index);
            BindExposedReference(newItem.FindPropertyRelative("item"), spawnPoint, director);
            newItem.FindPropertyRelative("chance").intValue = 10;
            newItem.FindPropertyRelative("isLocked").boolValue = false;
        }
